...
В репозитории есть пример unit-файла: `systemd/wg-bot.service`. Скопируйте его и включите службу:

```bash
sudo cp systemd/wg-bot.service /etc/systemd/system/wg-bot.service
sudo systemctl daemon-reload
sudo systemctl enable --now wg-bot.service
```

Проверьте статус:

```bash
sudo systemctl status wg-bot.service
```
